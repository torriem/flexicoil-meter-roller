...
translate([-50,-50,0])
	extra_fine_halfwidth(flute_depth, chamfer_angle=30);

translate([50,-50,0])
	canola_experimental(flute_depth, chamfer_angle=30);
...
/* An experimental segment design for canola that uses
   ...
   and only half width.
*/
module canola_experimental(flute_depth = extra_fine_flute_depth, chamfer_angle=20) {

	union() {
		segment_part(segment_width, segment_diameter_tight, 0, 25, top_chamfer = 3, chamfer_angle=chamfer_angle)
			blank(segment_width);

		translate([0,0,segment_width / 4])
			segment_part(segment_width, segment_diameter_tight, 0, 25, top_chamfer = flute_depth, chamfer_angle=chamfer_angle)
		...
		translate([0,0,segment_width / 4 * 2])
			segment_part(segment_width, segment_diameter_tight, 75, 100, bottom_chamfer = flute_depth, chamfer_angle = chamfer_angle)
				extra_fine_fluted(segment_width, spiral=true, flute_depth=flute_depth);
				//extra_fine(segment_width, spiral=true, scallop_depth=flute_depth);
...
module extra_fine_halfwidth(flute_depth = extra_fine_flute_depth, chamfer_angle = 20) {
